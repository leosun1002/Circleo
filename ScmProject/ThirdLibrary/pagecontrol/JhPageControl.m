...
-(void)createPointView{
    ...
    switch (_PageControlStyle) {
        ...
        {
            [self CreatDotWithSize:CGSizeMake(6, 6) withOtherDotSize:CGSizeMake(4, 4)];
        }
            break;
        default:
            break;
...
-(void)CreatDotWithSize:(CGSize)currentDotSize withOtherDotSize:(CGSize)otherDotSize{
    ...
    for (int page=0; page<_numberOfPages; page++) {
        if(page==_currentPage){
            UIView *currPointView=[[UIView alloc]initWithFrame:CGRectMake(startX, 0, currentDotSizeW, currentDotSizeH)];
