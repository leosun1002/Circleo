...
- (void)setupTableView
{
    ...
    //嵌套一层view   否则tableview有问题
    UIView *backview = [[UIView alloc] initWithFrame:CGRectMake(0, navBarHeight,self.view.bounds.size.width, self.view.bounds.size.height - navBarHeight)];
    backview.backgroundColor = NIMKit_UIColorFromRGB(0xF7F5FA);
    [self.view addSubview:backview];
    
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, backview.nim_height) style:UITableViewStylePlain];
    self.tableView.backgroundColor = NIMKit_UIColorFromRGB(0xF7F5FA);
